...
{
	cout<<"得分最高的人收益减半，得分最低的人收益加倍。"<<endl;
	cout<<"请输入游戏人数：";
	cin>>n;
	while(!(n>=2&&n<=16))
	{
...
void print_player_info(bool show_Round)
{
	...
	for(int i=1;i<=n;i++)
	{
		...
		player[i].flag_min=false;
		if(player[i].score>Max)
		{
			Max=player[i].score;
			Max_cnt=1;
		}
		else if(player[i].score==Max)
			Max_cnt++;
	...
	for(int i=1;i<=n;i++)
	{
		...
		{
			cout<<"收益状态：";
			if(player[i].flag_max)
				cout<<"减半";
			else if(player[i].flag_min)
				cout<<"加倍";
			else
				cout<<"正常";
...
int main()
{
	...
	while(1)
	{
		...
		{
			cout<<"你挖掉了木头！"<<endl;
			player[cur].score+=(player[cur].flag_min?2:1)*player[cur].mul/(player[cur].flag_max?20:10);
			game[x][y]=1;
		}
		else if(game[x][y]==5)
		{
		...
		{
			cout<<"你挖掉了煤矿！"<<endl;
			player[cur].score+=3*(player[cur].flag_min?2:1)*player[cur].mul/(player[cur].flag_max?20:10);
			game[x][y]=1;
		}
		else if(game[x][y]==7)
		{
			cout<<"你挖掉了铁矿！"<<endl;
			player[cur].score+=8*(player[cur].flag_min?2:1)*player[cur].mul/(player[cur].flag_max?20:10);
			game[x][y]=1;
		}
		else if(game[x][y]==8)
		{
			cout<<"你挖掉了金矿！"<<endl;
			player[cur].score+=24*(player[cur].flag_min?2:1)*player[cur].mul/(player[cur].flag_max?20:10);
			game[x][y]=1;
		}
		else if(game[x][y]==9)
		{
			cout<<"你挖掉了钻石！"<<endl;
			player[cur].score+=64*(player[cur].flag_min?2:1)*player[cur].mul/(player[cur].flag_max?20:10);
			game[x][y]=1;
		}
		else if(game[x][y]==10)
		{
