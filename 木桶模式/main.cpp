...
void print_player_info(bool show_Round)
{
	for(int i=1;i<=n;i++)
	{
		...
		cout<<"得分：min("<<player[i].score1<<","<<player[i].score2<<","<<player[i].score3<<") = "<<player[i].score<<" | ";
...
int main()
{
	...
	while(1)
	{
		...
		else if(game[x][y]==4)
		{
			...
			int s=player[cur].mul/10;
			
			if(player[cur].score1<=player[cur].score2&&player[cur].score1<=player[cur].score3)
				player[cur].score1+=s;
			else if(player[cur].score2<=player[cur].score1&&player[cur].score2<=player[cur].score3)
				player[cur].score2+=s;
			else
				player[cur].score3+=s;
			
			player[cur].score=min(min(player[cur].score1,player[cur].score2),player[cur].score3);
